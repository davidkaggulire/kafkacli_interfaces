...
# from . import IMessageBroker


class KafkaBrokerMock(IMessageBroker):
    ...
    def produce_method(args):
        ...
        data = KafkaBrokerMock.encode_func(args)

        return p.produce(channel, data, callback=KafkaBrokerMock.delivery_report)

    def flush_method(args):
        value = KafkaBrokerMock.produce_method(args)
        return value.flush()
    ...
    def sendMessages(self, args):
        ...
        p.produce(channel, data, callback=KafkaBrokerMock.delivery_report)
        p.flush()
    ...
    def readMessages(self, args):
        c = KafkaBrokerMock.consume_messages(args)
